...
@author: Levi Lingsch
to implement the 1D VNO on the time independent Burgers equation using data with an expanding distribution.
"""
...
class SpectralConv1d(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.matmul_real(out_ft, self.V_ct)
...
dataloader = MatReader('../../VNO_data/1d/exp_burgers_data_R10.mat')
...
# torch.save(model, '../VNO_models/exp_vandermonde_burgers')
...
################################################################
# model = torch.load('./VNO_models/exp_vandermonde_burgers')
# pred = torch.zeros(y_test.shape)
# index = 0
# test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_test, y_test), batch_size=1, shuffle=False)
# with torch.no_grad():
...
# save predictions
################################################################
scipy.io.savemat('../VNO_predictions/1d/exp_burger_test.mat', mdict={'pred': pred.cpu().numpy()})
